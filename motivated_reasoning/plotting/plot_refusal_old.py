"""
Refusal plotting script for model evaluation structures.

This script creates refusal over time plots comparing different model types.
"""
...
def plot_refusal_over_time(prompt_type: str, legend_shadow: bool = False):
    """Plot refusal over time comparing three different model types."""
    ...
    for run_name, config in run_configs.items():
        ...
        # Load data and calculate refusal for each iteration
        iterations = []
        avg_scores = []
        ...
        for iteration_name, eval_file in sorted(iteration_files.items(),
            ...
            if scores:
                ...
                # Calculate refusal as 1 - compliance
                avg_scores.append(1 - np.mean(scores))
    ...
    base_filename = 'model_comparison_response_refusal_over_time'
    refusal_dir = Path('plots') / 'refusal' / prompt_type
    output_file = refusal_dir / f'{base_filename}.png'
    ...
    pdf_file = refusal_dir / f'{base_filename}.pdf'
    ...
    for model_label, (iterations, avg_scores) in plot_data.items():
        ...
        print(f"  Refusal rates: {[f'{score:.3f}' for score in avg_scores]}")


def main():
    """Create both refusal over time plots: matched evaluation and CoT evaluation."""
    parser = argparse.ArgumentParser(description='Generate refusal over time plots')
    parser.add_argument('--legend-shadow', action='store_true',
                        help='Add shadow to legend (default: False)')
    args = parser.parse_args()

    print("="*80)
    print("CREATING REFUSAL OVER TIME PLOTS")
    print("="*80)

    print("\nCreating non-CoT evaluation plot...")
    plot_refusal_over_time(prompt_type='no_cot', legend_shadow=args.legend_shadow)

    print("\nCreating CoT evaluation plot...")
    plot_refusal_over_time(prompt_type='simple_cot', legend_shadow=args.legend_shadow)

    print("\nCreating Constitutional CoT evaluation plot...")
    plot_refusal_over_time(prompt_type='constitutional_cot', legend_shadow=args.legend_shadow)

    print("\nCreating matched evaluation plot...")
    plot_refusal_over_time(prompt_type='matched', legend_shadow=args.legend_shadow)

    print("\n" + "="*80)
    print("REFUSAL OVER TIME PLOTS COMPLETED!")
    print("="*80)
